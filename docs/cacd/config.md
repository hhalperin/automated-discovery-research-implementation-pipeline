...
All configuration lives in `.cacd/config.yaml`. Anything not set falls
...
```
cacd: allow-mixed we intentionally bumped node_modules to fix CVE
```
...
```
cacd: docs-skip purely internal refactor, no API change
```
...
The daily `cacd-cleanup.yml` workflow reads these.
