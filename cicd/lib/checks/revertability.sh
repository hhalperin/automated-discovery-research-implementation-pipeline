...
#     unless the PR body contains `cicd: allow-mixed <reason>`
#   * it touches at most one "intent zone" (source | docs | infra) unless
#     the PR body explicitly opts in
set -euo pipefail
. "$CICD_ROOT/lib/common.sh"

max_commits=$(cicd::config checks.revertability.max_commits 20)
vendored_patterns_json=$(cicd::config checks.revertability.vendored_patterns \
    '["^vendor/","/vendor/","^node_modules/","/node_modules/","package-lock.json$","yarn.lock$","poetry.lock$","Pipfile.lock$","go.sum$"]')
infra_patterns_json=$(cicd::config checks.revertability.infra_patterns \
...
body="$(cicd::pr_body)"
mixed_ok=0
grep -qE '^cicd: allow-mixed' <<< "$body" && mixed_ok=1
...
if [[ $has_vendored -eq 1 && $has_source -eq 1 && $mixed_ok -eq 0 ]]; then
    reasons+=("source + vendored/lockfiles mixed (add 'cicd: allow-mixed <reason>' to PR body to override)")
fi
...
# CICD itself) mixed with source/docs, because those are typically independently
# revertable concerns. Infra-only PRs are fine.
if (( has_infra == 1 && ( has_source == 1 || has_docs == 1 ) && mixed_ok == 0 )); then
    reasons+=("infra changes mixed with source/docs; split PR or add 'cicd: allow-mixed <reason>'")
fi
...
if (( ${#reasons[@]} > 0 )); then
    cicd::emit check revertability fail "$(IFS='; '; echo "${reasons[*]}")" "$details"
    exit 1
fi

cicd::emit check revertability pass "PR is cleanly revertable (${commits} commits, single intent)" "$details"
exit 0
