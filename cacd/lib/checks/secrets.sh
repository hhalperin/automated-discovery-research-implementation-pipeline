...
if [[ -z "$base" ]]; then
    cacd::emit check secrets skipped "cannot determine merge base" null
    exit 0
fi
...
# Limit to added lines; ignore .cacd/lib/checks/secrets.sh itself and any
# patterns-only file. Also ignore files matched by .cacd/secrets-allowlist.
allowlist_file="$CACD_REPO/.cacd/secrets-allowlist"

tmp=$(mktemp)
git diff --unified=0 "$base..$head" -- . ':(exclude)cacd/lib/checks/secrets.sh' ':(exclude).cacd/out/**' \
    | grep -E '^\+' \
    | grep -vE '^\+\+\+' \
    | grep -Ev 'cacd: secret-allow' \
    > "$tmp" || true
...
if [[ -n "$matches" ]]; then
    ...
    cacd::emit check secrets fail "possible secrets detected in $count added line(s)" "$details"
    rm -f "$tmp"
    exit 1
fi

rm -f "$tmp"
cacd::emit check secrets pass "no probable secrets in diff" null
exit 0
