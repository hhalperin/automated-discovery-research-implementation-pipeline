#!/usr/bin/env bash
# cacd/lib/new-repo.sh — scaffold a brand-new repo with CACD preinstalled.
#
# Usage:
...
#   * CACD installed (.github/workflows/cacd*.yml, .cacd/config.yaml, cacd/)
...
case "$language" in python|py|node|js|ts|generic) ;; *)
    cacd::err "unsupported language: $language (use py|node|generic)"
    exit 2
    ;;
esac
...
cp -r "$CACD_ROOT/templates/new-repo/common/." "$target_dir/"
if [[ -d "$CACD_ROOT/templates/new-repo/$language" ]]; then
    cp -r "$CACD_ROOT/templates/new-repo/$language/." "$target_dir/"
fi
...
# Install CACD into the new repo.
"$CACD_ROOT/install.sh" "$target_dir" --quiet
...
if (( no_git == 0 )); then
    (
        ...
        if [[ ! -d .git ]]; then
            git init -q -b main
            git add .
            git commit -q -m "chore: scaffold $name with CACD" || true
        fi
        if (( no_remote == 0 )) && [[ -n "$owner" ]] && command -v gh >/dev/null 2>&1; then
            gh repo create "$owner/$name" --private --source . --remote origin --push || cacd::err "gh repo create failed"
        fi
    )
fi

cacd::log "scaffolded $language repo at: $target_dir"
