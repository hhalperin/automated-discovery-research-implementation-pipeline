#!/usr/bin/env bash
# cacd/lib/report.sh — aggregate .cacd/out/{checks,judges}/*.json into:
#   * .cacd/out/report.md   (sticky PR comment body)
#   * .cacd/out/labels.txt  (one label per line, for GH labeler)
#   * .cacd/out/summary.json
set -euo pipefail
. "$CACD_ROOT/lib/common.sh"

summary_file="$CACD_OUT/summary.json"
report_file="$CACD_OUT/report.md"
labels_file="$CACD_OUT/labels.txt"

# Build a machine-readable summary.
python3 - "$summary_file" "$CACD_OUT/checks" "$CACD_OUT/judges" <<'PY'
...
# Render the sticky PR comment.
{
    ...
    case "$gate_status" in
        pass) echo "### CACD — all gates passed" ;;
        fail) echo "### CACD — one or more gates failed" ;;
        *)    echo "### CACD" ;;
    esac
    echo ""
    ...
    echo "Output artifacts: \`.cacd/out/\`. Re-run locally with \`cacd run\`."
} > "$report_file"

# Labels: one per line, only the most useful ones.
{
    gate_status=$(jq -r '.gate_status' <<<"$summary")
    [[ "$gate_status" == "pass" ]] && echo "cacd/passing" || echo "cacd/failing"
    jq -r '.checks[] | select(.status == "fail") | "cacd/gate-\(.name)-fail"' <<<"$summary"
    # Labels reflecting judge verdicts (advisory only).
    jq -r '.judges[] | select(.details.verdict == "regression" or .details.verdict == "fail") | "cacd/judge-\(.name)-regression"' <<<"$summary"
} | sort -u > "$labels_file"

cacd::log "report written: $report_file"
cacd::log "labels:       $labels_file"
cacd::log "summary:      $summary_file"
